...
#line 1 "uri.rl"
// usage: ragel -G2 -C uri.rl -o uri.rl.cc
...
#line 29 "uri.rl"
                {
                    if (p != mark) {
                        char* end = nullptr;
                        uri->setPort(strtol(mark, &end, 10));
                    }
                    mark = nullptr;
                }
                goto st453;
...
std::string Uri::toString() const {
    std::stringstream ss;
    ss << m_scheme << "://" << m_userinfo << (m_userinfo.empty() ? "" : "@") << m_host
       << (isDefaultPort() ? "" : ":" + std::to_string(m_port)) << getPath()
