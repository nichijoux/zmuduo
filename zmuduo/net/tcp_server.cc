...
{
    // 设置acceptor的新连接回调
    m_acceptor->setNewConnectionCallback([this](int socketFD, const Address::Ptr& peerAddress) {
...
void TcpServer::newConnection(int socketFD, const Address::Ptr& peerAddress) {
    ...
    // 获取本地的address
    auto         addrin       = sockets::getLocalAddress(socketFD);
...
void TcpServer::removeConnectionInLoop(const TcpConnectionPtr& connection) {
    ...
    // 从connections中移除name
    size_t n = m_connections.erase(connection->getName());
    assert(n == 1);
    // 获取connection的ioLoop
    auto ioLoop = connection->getEventLoop();
