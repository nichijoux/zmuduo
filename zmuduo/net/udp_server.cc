...
void UdpServer::send(const void* data, size_t length, const Address::Ptr& peerAddress) {
    EventLoop* ioLoop = m_threadPool->getNextLoop();
    ioLoop->runInLoop(
        [this, data, length, &peerAddress]() { sendInLoop(data, length, peerAddress); });
}

void UdpServer::sendInLoop(const void* data, size_t length, const Address::Ptr& peerAddress) {
    // 发送
...
void UdpServer::newMessage() {
    ...
    ssize_t     n           = ::recvfrom(m_socket.getFD(), const_cast<char*>(inputBuffer.peek()),
                                         inputBuffer.getWriteableBytes(), 0,
                                         reinterpret_cast<sockaddr*>(&sockaddrIn), &sockaddrLen);
    if (n > 0) {
        inputBuffer.hasWritten(n);
        auto peerAddress = Address::Create(reinterpret_cast<sockaddr*>(&sockaddrIn));
