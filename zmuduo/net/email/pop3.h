...
struct Pop3Response {
     ...
     */
    explicit Pop3Response(const bool success)
        : m_success(success) {}
     ...
     */
    Pop3Response(const bool success, std::string message)
        : m_success(success),
 ...
 */
struct Pop3StatResponse : Pop3Response {
    using Ptr = std::shared_ptr<Pop3StatResponse>;

    /**
     * @brief 默认构造函数。
     */
    Pop3StatResponse() = default;
     ...
     * @param[in] size 总字节数。
     */
    Pop3StatResponse(const int num, const size_t size)
        : Pop3Response(true),
          m_num(num),
          m_size(size) {}

    int    m_num  = 0; ///< 邮件总数
    size_t m_size = 0; ///< 总字节数
};
 ...
 */
struct Pop3UidlResponse : Pop3Response {
    using Ptr = std::shared_ptr<Pop3UidlResponse>;

    struct UidlItem {
        UidlItem(const int num, std::string id)
            : m_num(num),
              m_id(std::move(id)) {}
 ...
 */
struct Pop3UidlNResponse : Pop3Response {
    using Ptr = std::shared_ptr<Pop3UidlNResponse>;

    /**
     * @brief 默认构造函数。
     */
    Pop3UidlNResponse() = default;
     ...
     * @param[in] id 邮件唯一标识符。
     */
    Pop3UidlNResponse(const int num, std::string id)
        : Pop3Response(true),
          m_num(num),
          m_id(std::move(id)) {}
 ...
 */
struct Pop3ListResponse : Pop3Response {
    using Ptr = std::shared_ptr<Pop3ListResponse>;

    struct ListItem {
        ListItem(const int num, const size_t size)
            : m_num(num),
              m_size(size) {}
 ...
 */
struct Pop3ListNResponse : Pop3Response {
    using Ptr = std::shared_ptr<Pop3ListNResponse>;

    /**
     * @brief 默认构造函数。
     */
    Pop3ListNResponse() = default;
     ...
     */
    Pop3ListNResponse(const int num, const size_t size)
        : Pop3Response(true),
          m_num(num),
          m_size(size) {}

    int    m_num  = 0; ///< 邮件编号
    size_t m_size = 0; ///< 邮件大小（字节）
};
 ...
 */
struct Pop3RetrResponse : Pop3Response {
     ...
     */
    explicit Pop3RetrResponse(std::string message)
        : Pop3Response(true, std::move(message)) {}
};
 ...
 */
struct Pop3DeleResponse : Pop3Response {
     ...
     * @param[in] message 响应消息。
     */
    explicit Pop3DeleResponse(std::string message)
        : Pop3Response(true, std::move(message)) {}
};
 ...
 */
struct Pop3RsetResponse : Pop3Response {
     ...
     * @param[in] message 响应消息。
     */
    explicit Pop3RsetResponse(std::string message)
        : Pop3Response(true, std::move(message)) {}
};
 ...
 */
struct Pop3TopResponse : Pop3Response {
    ...
    explicit Pop3TopResponse(std::string header, std::string bodyPreview)
        : Pop3Response(true),
          ...
          m_bodyPreview(std::move(bodyPreview)) {}
 ...
 */
struct Pop3NoopResponse : Pop3Response {
    using Ptr = std::shared_ptr<Pop3NoopResponse>;

    /**
     * @brief 构造函数。
     */
    explicit Pop3NoopResponse()
        : Pop3Response(true) {}
};
 ...
 */
struct Pop3CapaResponse : Pop3Response {
 ...
 */
struct Pop3QuitResponse : Pop3Response {
     ...
     * @param[in] message 响应消息。
     */
    explicit Pop3QuitResponse(std::string message)
        : Pop3Response(true, std::move(message)) {}
};
} // namespace zmuduo::net::email
