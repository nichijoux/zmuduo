...
HttpClient::HttpClient(EventLoop* loop, const std::string& uri, std::string name)
    : HttpClient(loop, common_util::CheckNotNull(Uri::Create(uri)), std::move(name)) {}
...
void HttpClient::doRequest(const HttpMethod             method,
...
void HttpClient::onConnection(const TcpConnectionPtr& connection) {
    ...
    if (connection->isConnected()) {
        ...
        connection->send(std::get<HttpRequest>(node).toString());
        if (auto timeout = std::get<uint32_t>(node); timeout != 0) {
        ...
        if (connection->getContext().has_value()) {
            ...
            if (auto& parser = context->getResponseParser(); parser.needForceFinish()) {
                if (auto response = parser.getResponse();
                    response.getHeader("Content-Length").empty() &&
                    response.getHeader("Transfer-Encoding").empty()) {
                    parser.forceFinish();
                    // 获取回调
                    auto item = m_callbacks.front();
                    m_callbacks.pop();
                    std::get<HttpResponseCallback>(item)(
                        std::make_optional<HttpResponse>(context->getResponse()));
                }
            }
        }
        if (m_reconnect && !m_callbacks.empty()) {
            ...
            while (!m_callbacks.empty()) {
                auto item = m_callbacks.front();
                m_callbacks.pop();
                std::get<HttpResponseCallback>(item)(std::nullopt);
            }
        }
    }
}
...
needParse:
    auto context = std::any_cast<HttpContext::Ptr>(connection->getContext());
    // 解析响应
    if (int code = context->parseResponse(buffer); code == 1) {
        if (m_timerId) {
        ...
        if (!m_callbacks.empty() && !response.isClose()) {
            m_client.send(std::get<HttpRequest>(item).toString());
            // 定时器
            if (auto timeout = std::get<uint32_t>(item); timeout != 0) {
                m_timerId = std::make_unique<TimerId>(m_client.getEventLoop()->runAfter(
                    timeout, [this] { m_client.disconnect(); }));
            }
        }
        // 回调
        std::get<HttpResponseCallback>(item)(std::make_optional<HttpResponse>(response));
        // 重置context
        connection->setContext(std::make_shared<HttpContext>());
        // 是否需要关闭
