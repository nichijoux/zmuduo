...
Timestamp Timestamp::FromUnixTime(const time_t t, const int microseconds) {
    return Timestamp(t * S_MICRO_SECONDS_PER_SECOND + microseconds);
}
...
std::string Timestamp::toString() const {
    // 1. 将微秒时间戳转换为秒级时间戳
    const auto seconds      = m_microSecondsSinceEpoch / S_MICRO_SECONDS_PER_SECOND;
    const int  microseconds = static_cast<int>(
        m_microSecondsSinceEpoch % S_MICRO_SECONDS_PER_SECOND);
...
time_t Date::toDateEpoch() const {
    // 1. 将微秒时间戳转换为秒级时间戳
    const auto seconds = m_microSecondsSinceEpoch / 1000000;
