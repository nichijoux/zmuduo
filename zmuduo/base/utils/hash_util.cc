...
std::string HashUtil::Base64decode(std::string_view src) {
...
std::string HashUtil::Base64encode(const void* data, size_t length) {
...
std::string HashUtil::HexToBinary(std::string_view hex) {
...
std::string HashUtil::SHA1sum(const void* data, size_t length) {
...
std::string
HashUtil::MD5(const std::string& input, int bitLength /* = 32 */, bool toUpper /* = false */) {
    // 初始化
...
std::string HashUtil::RandomString(size_t length) {
    // 使用随机设备作为种子
...
void HashUtil::SHA1Transform(uint32_t state[5], const uint8_t buffer[64]) {
    uint32_t a, b, c, d, e;
    uint32_t w[80];
...
std::string HashUtil::SHA1Final(const uint32_t state[5]) {
...
void HashUtil::md5Process(const uint8_t* block,
                          uint32_t&      A,
                          uint32_t&      B,
                          uint32_t&      C,
                          uint32_t&      D) {
