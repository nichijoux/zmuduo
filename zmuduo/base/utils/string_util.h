 ...
 *
 */
class StringUtil : NoMoveable, NoCopyable {
  public:
    /**
     * @brief URL编码函数
     *
     * 将字符串编码为URL安全格式（遵循RFC 3986标准）
     * 可选是否将空格编码为"+"（常用于表单数据处理）
     *
     * @param str 需要编码的原始字符串
     * @param spaceAsPlus 是否将空格编码为"+"（默认false）
     * @return std::string URL编码后的字符串
     *
     * @note 保留字符（A-Za-z0-9-_.~）不会被编码
     * @example
     * @code
     * auto result1 = UrlEncode("Hello World!")
     * // 返回 "Hello%20World%21"<br/>
     * auto result2 = UrlEncode("Name Value", true)
     * // 返回 "Name+Value"
     * @endcode
     */
    /**
     ...
     */
    static std::string
    UrlEncode(const std::string& url, bool isFullUrl = true, bool spaceAsPlus = false);
    /**
     * @brief URL解码函数
     *
  ...
  private:
    /**
     * @brief 编码 URL 片段（路径、查询、片段）
     *
     * @param str 要编码的字符串片段
     * @param spaceAsPlus 是否将空格编码为 '+'（否则为 "%20"）
     * @param encodeSlash 是否编码 '/' 字符（false 用于 path，true 用于 query/fragment）
     * @return 编码后的字符串
     *
     * @note 本函数用于内部处理 URL 各组成部分的编码逻辑
     */
    static std::string UrlEncodeCore(const std::string& str, bool spaceAsPlus, bool encodeSlash);
};
}  // namespace zmuduo::utils
