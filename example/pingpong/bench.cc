...
std::pair<int, int> runOnce() {
    ...
    for (int i = 0; i < numPipes; ++i) {
        ...
        channel.setReadCallback([capture0 = channel.getFD(), i](auto&& PH1) {
            readCallback(std::forward<decltype(PH1)>(PH1), capture0, i);
        });
    ...
    int iterTime =
        static_cast<int>(end.getMicroSecondsSinceEpoch() - beforeInit.getMicroSecondsSinceEpoch());
    int loopTime =
        static_cast<int>(end.getMicroSecondsSinceEpoch() - beforeLoop.getMicroSecondsSinceEpoch());
...
int main(int argc, char* argv[]) {
    ...
    while ((c = getopt(argc, argv, "n:a:w:")) != -1) {
        switch (c) {
            case 'n': numPipes = lexical_cast<int>(optarg); break;
            case 'a': numActive = lexical_cast<int>(optarg); break;
            case 'w': numWrites = lexical_cast<int>(optarg); break;
    ...
    for (int i = 0; i < numPipes; ++i) {
        auto* channel = new Channel(&loop, g_pipes[i * 2]);
